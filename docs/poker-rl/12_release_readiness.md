...
| M6 — эксплуатационная готовность | **Частично** | Versioned model/full-run checkpoints: `poker/model.py`, `poker/train_runner.py`; atomic save, optimizer/RNG/league resume и finite-health gate; hash-проверяемые HU/curriculum evidence; immutable per-iteration ledger, read-only health summary и multi-seed campaign selection: `poker/experiments.py`, `poker/experiment_summary.py`, `poker/tuning.py`, `poker/campaign.py`; pinned opponent registry and player-safe aggregate reports. | Нет опубликованного evaluation report реального обученного canonical checkpoint'а и versioned HTTP API (`/api/v1/...`). Smoke проверяет pipeline, а не силу стратегии. |
...
1. Выполнить preregistered multi-seed campaign stage A (heads-up) с
   зафиксированными конфигурацией, seed matrix и бюджетом; сохранить campaign
   report, затем обучить canonical candidate выбранного варианта.
